...
export default function Cart() {
  ...
  let {getcart,produtsCart,updateCartQuantity,deleteItem,totalPrice,numberOfCart,price}=useContext(CartContext)
...
<div className="relative overflow-x-auto shadow-md sm:rounded-lg">
  <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
    ...
    {produtsCart?.map((product)=> <tr key={product.product._id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
        ...
        <td className="px-6 py-4 font-semibold text-gray-900 dark:text-white">
        {product.price*product.count}         </td>
        <td className="px-6 py-4">
          <span onClick={()=>deleteItem(product.product._id)} href="#" className="font-medium text-red-600 dark:text-red-500 hover:underline">Remove</span>
        </td>
      </tr>
)}
    </tbody>
...
<div className='my-9'>
  <Link className='btn' to='/payment'> payment  </Link>
</div>
    </div> 
  )
}
